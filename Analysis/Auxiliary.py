...
def clean(df, z_roxInitial_exclude=2.5, z_fqInitial_exclude=2.5, frac_edge_exclude=0.05, dropna=True):
    ...
    for set in df['set'].unique():
        ...
        df = df.drop(df.loc[(df['set'] == set) & ((df['x'] < new_x_min) | (df['x'] > new_x_max) | (df['y'] < new_y_min) | (df['y'] > new_y_max))].index)
...
def getClassifier(path_train, label_frame, classify_frame, mt_sets, wt_sets, neg_prob_cutoff, features_norm, features_orig, n_class_members):
    ...
    X_TRAIN = np.array(df_train.query('UID in @uid_train')[['{}_corr'.format(feat) if feat in features_norm else feat for feat in features]])
